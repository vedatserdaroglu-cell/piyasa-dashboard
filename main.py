...
Streamlit Cloud'da sürekli canlı çalışacak şekilde optimize edilmiştir.
...
Deployment için:
    ...
    2) share.streamlit.io → Deploy → repo/branch/main.py seç
    3) Canlı!

Yazar: Quant Dashboard v2.0
...
# ═══════════════════════════════════════════════════════════════
# BÖLÜM 3: HATA YÖNETİMİ — DEKORATÖR & YARDIMCI FONKSİYONLAR
# ═══════════════════════════════════════════════════════════════
...
def safe_fetch(func):
    ...
    kullanıcıya şık bir uyarı gösterir, sistemi çökertmez.
    """
    def wrapper(*args, **kwargs):
        try:
            ...
            st.warning(
                f"⚠️ **Bağlantı Hatası** — `{func.__name__}` çalıştırılırken "
            ...
            st.warning(
                ...
                f"Sunucu yoğun olabilir, birkaç dakika sonra tekrar deneyin.",
                icon="⏱️"
            )
            return None
        except Exception as e:
            st.error(
                f"❌ **Beklenmeyen Hata** — `{func.__name__}` → {type(e).__name__}: {str(e)[:200]}",
                icon="🔧"
            )
            return None
    return wrapper
...
def show_data_warning(indicator_name: str):
    """Veri mevcut olmadığında gösterilecek uyarı bloğu."""
    st.info(
        f"📭 **{indicator_name}** verisi bu analiz döngüsünde alınamadı. "
        f"Bir sonraki yenilemede tekrar denenecek.",
        icon="📭",
    )


# ═══════════════════════════════════════════════════════════════
# BÖLÜM 4: VERİ ÇEKME MOTORU (THREADING İLE PARALEL)
# ═══════════════════════════════════════════════════════════════

def _fetch_one(ticker: str, start: str, end: str) -> Tuple[str, Optional[pd.DataFrame]]:
    """Tek hisse verisini yfinance ile indirir. ThreadPool içinde çağrılır."""
    try:
...
def fetch_universe_parallel(
    tickers: List[str], start: str, end: str, max_workers: int = 20
) -> Dict[str, pd.DataFrame]:
    """
    Hisse evrenini paralel olarak çeker.
    ThreadPoolExecutor ile ~20 eşzamanlı bağlantı.
    Streamlit Cloud'un kaynak limitlerini aşmamak için optimize edildi.
    """
    results = {}
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(_fetch_one, t, start, end): t for t in tickers}
...
def fetch_index_etf(ticker: str, start: str, end: str) -> Optional[pd.DataFrame]:
    """Endeks ETF verisini (SPY/QQQ) çeker."""
    df = yf.download(ticker, start=start, end=end, progress=False, auto_adjust=True)
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)
...
def fetch_macro_instruments(start: str, end: str) -> Dict[str, Optional[pd.DataFrame]]:
    """VIX, 10Y Treasury, 13W Treasury verilerini çeker."""
    instruments = {"^VIX": "VIX", "^TNX": "TNX", "^IRX": "IRX"}
    result = {}
    for yf_ticker, label in instruments.items():
        try:
            df = yf.download(yf_ticker, start=start, end=end, progress=False, auto_adjust=True)
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.get_level_values(0)
            result[label] = df if not df.empty else None
        except Exception:
            result[label] = None
    return result
...
def calc_distribution_days(idx_df: pd.DataFrame, lookback: int = 25) -> pd.DataFrame:
    """
    IBD Distribution Day hesaplama:
    - Fiyat >= %0.2 düşüş VE hacim önceki günden yüksek
    - Son 25 günde toplam kaç dağıtım günü var (kayan pencere)
    """
    df = idx_df.copy()
...
def calc_follow_through(idx_df: pd.DataFrame) -> pd.DataFrame:
    """
    Follow-Through Day: Rally 4+ gün, günlük artış >= %1.25, hacim artışı.
    """
    df = idx_df.copy()
    df["PctChg"] = df["Close"].pct_change() * 100
    df["ConsUp"] = 0
...
def calc_advance_decline(
    ...
    """
    Günlük Advances / Declines sayılarını ve kümülatif A/D Line'ı hesaplar.
    """
    # Her hissenin günlük return'ünü önceden hesapla (hız optimizasyonu)
    returns = {}
    ...
    for date in date_range:
        ...
        for ticker, ret_series in returns.items():
            if date in ret_series.index:
                val = ret_series.get(date, np.nan)
                if pd.notna(val):
                    if val > 0: adv += 1
                    elif val < 0: dec += 1
...
def calc_pct_above_ma(
    ...
    """Belirtilen MA'nın üzerindeki hisse yüzdesini günlük hesaplar."""
    # Önceden tüm MA'ları hesapla
    ma_cache = {}
    for ticker, df in all_data.items():
...
def calc_zbt(ad_df: pd.DataFrame) -> pd.DataFrame:
    """
    ZBT Ratio = Advances / (Advances + Declines)
    Sinyal: 10-gün EMA 0.40 altından 0.615 üzerine çıkarsa.
    """
    df = ad_df.copy()
...
def calc_new_highs_lows(
    ...
    """Her gün 52-haftalık zirve ve dip yapan hisselerin farkı."""
    records = []
    for date in date_range:
        nh, nl = 0, 0
        for ticker, df in all_data.items():
...
def calc_fear_greed_score(
    ...
    """
    0-100 arası Fear & Greed skoru hesaplar.
    0 = Extreme Fear, 100 = Extreme Greed.
    Returns: (score, label, color_hex)
    """
    ...
    components = [
        norm(pct50, 0, 100),                    # Breadth 50
        norm(pct200, 0, 100),                    # Breadth 200
        norm(mcclellan, -150, 150),              # McClellan
        norm(dist_count, 0, 8, invert=True),     # Dist Days (düşük = iyi)
        norm(vix, 10, 45, invert=True),          # VIX (düşük = iyi)
        norm(net_hl, -40, 40),                   # Net Highs-Lows
    ]
    score = int(np.mean(components))

    if score >= 75:
        return score, "Extreme Greed", C["green"]
    elif score >= 55:
        return score, "Greed", "#66bb6a"
    elif score >= 45:
        return score, "Neutral", C["yellow"]
    elif score >= 25:
        return score, "Fear", C["orange"]
    else:
        return score, "Extreme Fear", C["red"]
...
def determine_market_health(
    ...
    IBD tarzı piyasa sağlığı durumu.
    """
    score = 0
    ...
    if score >= 5:
        return "CONFIRMED UPTREND", "health-uptrend"
    elif score >= 2:
        return "UPTREND UNDER PRESSURE", "health-pressure"
    else:
        return "MARKET IN CORRECTION", "health-correction"


# ═══════════════════════════════════════════════════════════════
# BÖLÜM 6: PLOTLY GRAFİK FONKSİYONLARI (DARK THEME)
# ═══════════════════════════════════════════════════════════════

def _dark_layout(fig: go.Figure, title: str = "", h: int = 420) -> go.Figure:
    """Tüm grafiklere ortak dark layout uygular."""
    fig.update_layout(
...
# ─── SPY + A/D Line Overlay ───

def chart_price_with_breadth(
    idx_df: pd.DataFrame, ad_df: pd.DataFrame, idx_name: str
) -> go.Figure:
    ...
    # Panel 1: Fiyat + DMA'lar
    fig.add_trace(go.Scatter(
        x=idx_df.index, y=idx_df["Close"],
        name=idx_name, line=dict(color=C["blue"], width=1.8),
    ), row=1, col=1)
    ...
    if "AD_Line" in ad_df.columns:
        fig.add_trace(go.Scatter(
            x=ad_df.index, y=ad_df["AD_Line"],
            name="A/D Line", line=dict(color=C["cyan"], width=2),
        ), row=2, col=1)
    ...
    if "McClellan" in ad_df.columns:
        ...
        fig.add_trace(go.Bar(
            x=ad_df.index, y=ad_df["McClellan"],
            name="McClellan Osc.", marker_color=mc_colors, opacity=0.85,
        ), row=3, col=1)
...
def chart_distribution(dist_df: pd.DataFrame, name: str) -> go.Figure:
    fig = make_subplots(
...
def chart_ftd(ftd_df: pd.DataFrame, name: str) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(
...
def chart_pct_above(p50: pd.DataFrame, p200: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
...
def chart_coppock(cop_df: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
...
def chart_zbt(zbt_df: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(
...
def chart_highslows(hl_df: pd.DataFrame) -> go.Figure:
...
def chart_vix(vix_df: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(
...
def chart_yield(yc_df: pd.DataFrame) -> go.Figure:
...
def chart_radar(metrics: dict) -> go.Figure:
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        for sheet_name, df in data.items():
            if df is not None and isinstance(df, pd.DataFrame) and not df.empty:
                df.to_excel(w, sheet_name=sheet_name[:31])  # Excel max 31 karakter
    buf.seek(0)
    return buf
...
def run_pipeline(index_choice: str, start_str: str, end_str: str) -> Optional[Dict]:
    """
    Tüm analizi çalıştırır. 15 dakika (ttl=900) boyunca cache'lenir.
    Cache key = (index_choice, start_str, end_str).
    """
    try:
        ...
        # 2) Paralel hisse verisi
        all_data = fetch_universe_parallel(tickers, start_str, end_str, max_workers=20)
...
with st.sidebar:
    st.markdown(
        ...
        f"<p style='color:{_m};font-size:0.85rem;'>Quant Dashboard v2.0</p>",
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    st.markdown(
        ...
        f"📐 yfinance • Plotly • Streamlit<br>"
        f"Threading ile paralel veri<br>"
...
if R is None:
    st.markdown(
        ...
        İnternet bağlantınızı ve yfinance erişimini kontrol edin.
        </p></div>""",
        unsafe_allow_html=True,
    )
    st.stop()
...
# ── Son değerleri al (güvenli) ──
def safe_last(df, col, default=0):
    """DataFrame'den son değeri güvenli şekilde alır."""
    try:
...
with col_banner:
    ...
    st.markdown(
        ...
        f'Son güncelleme: {R["timestamp"]} • {R["fetched_count"]} hisse analiz edildi'
        f'</p>',
        unsafe_allow_html=True,
    )
...
with m2:
    st.metric("Dist. Days", f"{dist_count}",
              "DANGER" if dist_count >= 5 else "OK",
              delta_color="inverse")
with m3:
...
with tabs[0]:
    ...
    with col_b:
        ...
        **Distribution Day:**
        ...
        25 günde **5+** = <span style="color:{C['red']};">tehlike bölgesi</span>.
...
with tabs[3]:
    ...
    if vix_df_raw is not None and not vix_df_raw.empty:
        ...
        v = vix_df_raw["Close"]
        with vc1:
            st.metric("VIX Son", f"{v.iloc[-1]:.2f}")
        with vc2:
            st.metric("VIX Ort.", f"{v.mean():.2f}")
        with vc3:
            st.metric("VIX Min", f"{v.min():.2f}")
        with vc4:
            st.metric("VIX Max", f"{v.max():.2f}")
...
if live_mode:
    # Streamlit'in auto-rerun mekanizmasını kullan
    # 15 dakikada bir (900 saniye) otomatik yenileme
    # st.rerun() tetiklemek için session_state ile zamanlama
    ...
    st.markdown(
        ...
        f'Sonraki otomatik yenileme: {int(remaining // 60)}dk {int(remaining % 60)}sn'
        f'</p>',
        unsafe_allow_html=True,
    )
